...
This writes `worlds/<slug>/handoff/rhino/` with a collider GLB, original Gaussian splat PLY for SuperSplat, RGB point-cloud PLY for Rhino, panorama, manifest, and README. The default point cloud is 500k for visual fidelity; use `--density 150k` or `--density 100k` when import performance matters more.
...
- For Rhino/SuperSplat workflows, report the handoff folder and clarify that the GLB is geometry context, the RGB PLY is for Rhino, and the original splat PLY is for SuperSplat.
